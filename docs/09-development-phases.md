...
| **Build** | `ProviderAdapter` interface + all capability contracts · normalised DTOs · `ProviderRegistry` · encrypted credential storage + masked display + permission gating · provider CRUD (all 11 fields), budgets, rate limits, maintenance mode · model catalog CRUD (all 10 fields, 5 statuses) · capability flags · `ai_model_prices` with provider cost vs credit price and effective dating · `ModelSyncService` + Refresh Models + scheduled sync + sync logs · **API test console (§25)** · **AI provider diagnostics: connectivity, authentication, model catalog freshness — contributed by each adapter** · `OpenAiCompatibleAdapter` and `CustomHttpAdapter` |
...
| **Between 8a and 8b** | The three production blockers found by the readiness audit were closed first: the checkout button that opened nothing, signed links that would have 403'd behind a proxy, and email delivered to a log file — plus secure cookies, security headers, a debug-off check that had never executed, and a backup runbook with a performed restore. Recorded in `docs/12-decision-log.md` under "Production blockers", and in `docs/19-backup-and-restore.md`. |
...
| **8b — built** | Image generation (§16). One row per image with batch, lineage and retention; `SupportsImageGeneration` on the OpenAI-compatible adapter (so most of the market needs no code) and on Gemini, whose `:predict` shape differs in every part; `ImageService` refusing before anything is queued, one credit hold settled to what actually arrived; `FileStorage::storeGenerated()` typing bytes from their content against a list the panel cannot widen; the one inline-rendering route, behind four conditions; the customer studio and gallery; admin screens for settings, generations and retention. Gate: `npm run test:images` plus 44 PHP tests. |
| **8c — built** | Voice (§18). `SupportsTranscription` and `SupportsSpeech`, with Gemini's inline-audio transcription as the fourth wire shape; one `voice_jobs` table for both directions; quotas in minutes covering both; the recording stored before the provider is called; the transcript landing in the composer and never being sent on the customer's behalf; `per_second` pricing finally read by the cost calculator. Gate: `npm run test:voice` plus 28 PHP tests. |
